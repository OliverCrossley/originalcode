...
void DLASystem::moveLastParticle() {
    ...
    // check if destination is empty
        ...
        if (checkStick()) {
            ...
            // special print to file case for 10 particles
            if (numParticles == 10 && logfile.is_open()) {
                logfile << numParticles << " " << clusterRadius << endl;
            }

            // print to file every 100 particles
...
int DLASystem::checkStick() {
    ...
    for (int i = 0; i < 4; i++) {
        int max = 100;
        int random = rgen.randomInt(max); //generating a random number from 0 to 99
        double checkpos[2];
        setPosNeighbour(checkpos, lastP->pos, i);
        // if the neighbour is occupied...
        if (readGrid(checkpos) == 1)
            if (random < 9)    // (sticking percentage)
                result = 1;
    }
    return result;
}
...
DLASystem::DLASystem(Window* set_win) {
    ...
    logfile.open("opfile.txt", ios::app);
}
